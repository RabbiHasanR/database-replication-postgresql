...
- **Defines Two Services:**  
  ...
  - `replica`: The PostgreSQL standby node, built from `replica/Dockerfile`.
...
- **Service Names as Hostnames:**  
  Docker Compose automatically provides service names (`primary`, `replica`) as internal DNS hostnames, so the replica can reliably connect to the primary.
...
4. **Test replication:**
    - Insert data into the primary:
      ...
      ```
    - Check data on the replica:
      ```sh
      docker exec -it replica psql -U postgres -c "SELECT * FROM test_replication;"
      ```
